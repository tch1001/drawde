...
- Name: viewer.spec.ts >> drawde viewer >> 7 · clicking the ✕ on a rect removes it and its panel card
- Location: tests/viewer.spec.ts:220:3
  ...
  - banner [ref=e4]:
    ...
    - generic [ref=e6]:
      ...
      - button "T Text T" [ref=e10] [cursor=pointer]:
        - text: T Text
        - generic [ref=e11]: T
      - generic [ref=e12]: drag a box around an equation · Shift add · Ctrl+scroll zoom · PgUp/PgDn page
      - generic [ref=e13]:
  ...
  - main [ref=e18]:
    ...
    - complementary [ref=e145]:
      ...
      - generic [ref=e151]:
        ...
        - generic [ref=e157]: 250×50 pt·→ OCR / LLM (phase 3)
...
```ts
  140 |     // give async plugin work a beat to surface any late errors
  141 |     await page.waitForTimeout(1000);
  142 |     expect(errors, `unexpected console errors:\n${errors.join('\n')}`).toEqual([]);
  143 |   });
  144 | 
  145 |   test('2 · pressing R activates region mode', async ({ page }) => {
  146 |     await boot(page);
  147 |     const region = page.locator('.dd-mode').nth(0);
  148 |     await expect(region).not.toHaveClass(/\bon\b/);
